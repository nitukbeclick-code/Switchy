...
### גל D — יכולות צוות שחסרו (Tier 2)

| # | פריט | מצב | פירוט |
|---|------|-----|--------|
| D.1 | ✅ **שחרור והעברה של ליד משויך** | בוצע 2026-07-27 | `claimLead` מוגן `claimed_by=is.null` — מה שהפך שיוך ל**דלת חד-כיוונית**: שום action ב-crm-api לא יכול היה לנקות את העמודה, ולכן נציג שעזב/יצא לחופשה החזיק את כל הספר שלו לצמיתות ולמנהל לא היה שום מסלול תיקון. נוספו `releaseLead` (מחזיר למאגר) ו-`assignLead` (מעביר לנציג אחר, **ללא** ה-guard — במכוון: שיוך הוא כל-הקודם-זוכה, העברה ע"י מנהל היא הוראה). שניהם `admin_only` ב-`ACTION_CAP` **ולא** `write_leads`: `leads.claimed_by` הוא מחרוזת-תצוגה חופשית ללא uid מאחוריה, ולכן "שחרר את הליד שלך" אינו ניתן לאימות מזהות הקורא — ו-`write_leads` כאן היה אומר שכל נציג יכול לקחת ליד מכל נציג אחר. שניהם כותבים `lead_events` + audit של Reg.13 עם **ממי** נלקח הליד. ב-UI: פקדים במגירת הליד, מגודרים ב-`canAdmin` מאותו `whoami` שמסתיר את הטאבים, ומשתמשים ב-`CrmFetch` המוטָפס כדי להציג את הודעת השרת האמיתית (403/404/409) במקום "הפעולה נכשלה". |
| D.2 | 🔴 **שינויי ליד לא זורמים ב-realtime** | חסום על מיגרציה | `logCrmEvent` נקרא מ-4 מקומות בלבד, כולם ב-`actions_conversations.ts`. אף מוטציית ליד לא כותבת `crm_events`, ולכן ליד חדש / שיוך של נציג אחר / שינוי סטטוס אינם משודרים. **הסיבה שלא נסגר עם D.1:** ל-`crm_events` אין עמודת `lead_id` כלל (`crm-takeover-2026-06.sql:49`) — רק `conversation_id`/`contact_id`. כתיבה משם הייתה יוצרת שורות יתומות עם שני nulls. דורש מיגרציה שמוסיפה `lead_id`, ולכן אישור בעלים. |
