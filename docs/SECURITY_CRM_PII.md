...
`actions_leads.ts`. Phone keeps the last 3 digits (`•••••••567`); email keeps
...
`kind` ∈ `lead | contact | conversation | meeting`. Every call writes a
...
`crm_rate_limited` to the audit log and returns `429` + `Retry-After`.
