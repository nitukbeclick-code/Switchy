...
/// סיומת יחידת החשבון/תקציב לפי קטגוריה (הצורה הקצרה עם '/'), לתצוגת חשבון
/// חודשי ותקציב שאלון: '/חבילה' לחו"ל (החשבון שם הוא לרוב per-package), '/חודש'
/// לכל השאר. מקור אמת יחיד — מסכים לא גוזרים את הסיומת שוב מהקטגוריה (בעבר זה
/// נעשה בשלושה מקומות שנפרדו: השאלון לא הציג סיומת לחו"ל בכלל).
String billUnitLabel(String cat) => cat == 'abroad' ? '/חבילה' : '/חודש';
