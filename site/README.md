...
`triple.html`, `abroad.html` are generated by `build.js` from a data array +
shared template (unique title/description/H1/FAQ + JSON-LD per category). Edit
the data in `build.js` and run `node build.js` to regenerate them and refresh
`sitemap.xml`. `og-card.html` is the source for the rendered `og-image.png`.
