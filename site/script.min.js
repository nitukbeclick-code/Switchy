...
try { if (typeof window.gtag === 'function') window.gtag('event', name, props || undefined); } catch (_) { }
...
try { console.warn('CHOSECH: window.CHOSECH_SUPABASE is not configured — the lead was NOT sent to a backend.'); } catch (_) { }
...
if ($('referralShare')) return;
const cta = form.closest('.cta__inner, .container') || form.parentElement;
if (!cta) return;
let code = '';
try { code = (sessionStorage.getItem('chosechRef') || ''); } catch (_) { }
if (!code) {
code = Math.random().toString(36).slice(2, 8);
try { sessionStorage.setItem('chosechRef', code); } catch (_) { }
}
...
if (!chip || chip.hasAttribute('data-plan-reset')) return;
...
if (!res.ok) {
let detail = '';
try { const j = await res.json(); detail = j.message || j.error || j.hint || ''; } catch (_) { }
...
const minLead = Date.now() + 4 * 60 * 60 * 1000;
...
if (inst.getTime() < minLead) continue;
if (inst.getTime() > maxAhead) continue;
...
let pinned = null;
try { pinned = localStorage.getItem(KEY); } catch (_) { }
...
const grant = () => { try { if (typeof window.gtag === 'function') window.gtag('consent', 'update', { analytics_storage: 'granted' }); } catch (_) { } };
...
try { window.SwitchyI18n.init(); } catch (_) { }
const btn = document.getElementById('langBtn');
if (btn) { try { window.SwitchyI18n.mountMenu(btn); } catch (_) { } }
})();
(function () {
...
try { localStorage.setItem('switchy-booked', '1'); } catch (_) { }
return;
}
...
const save = (list) => { try { localStorage.setItem(KEY, JSON.stringify(list.slice(-40))); } catch (_) { } };
const paint = () => {
const list = read();
