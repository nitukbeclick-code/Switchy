// ─────────────────────────────────────────────────────────────────────────────
// _shared/scoring.ts — THE single, provider-neutral plan-ranking brain.
//
// This is the ONE source of truth that kills cross-surface ranking drift. It
// reconciles the two formulas that existed before:
//
//   1. site-plan-advisor/lib.ts  — a flat, additive price+feature score with a
//      provider-neutral random tie-break (the E-E-A-T "no hidden brand bias"
//      guarantee). Cheap, but coarse: one big price term + a few +40/+30 bumps.
//
//   2. lib/services/recommendation_engine.dart — a weighted, explainable 0..100
//      score built from six normalized sub-scores (price/saving/rating/speed/
//      coverage/flex), tuned by the user's stated priority, plus needs-met
//      bonuses, a budget-overrun penalty, and Hebrew reasons/caveats.
//
// MERGE: we keep the Dart engine's structure (it is the richer, explainable one
// the app already ships) and fold in the site formula's hard guarantees:
//   • PROVIDER NEUTRALITY — scorePlan never looks at the provider; rankPlans
//     breaks score ties with a DETERMINISTIC shuffle (seeded Fisher–Yates) so no
//     brand gets a structural edge AND the order is reproducible across surfaces
//     for the same inputs (a real Math.random tie-break would make WhatsApp and
//     the site disagree on equal-score ties). Higher scores always win; only
//     genuine ties are shuffled.
//   • HONEST RATINGS — a plan's rating is a real signal only once reviews > 0;
//     otherwise we use a neutral 0.6 midpoint (no fabricated social proof).
...
// site snapshot (site/data/plans.json) or the live public.plans table (mapped via
// _shared/catalogue.ts plansFromRows). Unknown fields are simply ignored.
export type ScorablePlan = {
  ...
  plan?: string;
  price?: number;
  after?: number | null; // post-promo monthly price (kamaze-parity); null = no step-up
  ...
  rating?: number; // 0..5 — only a real signal when reviews > 0 (see _ratingSignal)
...
// advisor priorities, normalized to one set. `data` and `service` are folded in
// (data → price-leaning balanced; service → rating-leaning) so every surface's
// priority id maps here without a separate enum.
...
// A snapshot of the user's needs, fed to the engine to score plans. This is the
// EXPORTED shared type downstream agents construct (from a site quiz, a WhatsApp
// slot context, or the app's AppState).
export type MatchProfile = {
  category: string; // cellular | internet | tv | triple | abroad ('' = any)
  currentBill?: number; // current monthly spend (per-package for abroad); 0/undef = unknown
...
// A scored plan: the 0..100 match score, the concrete annual saving (₪/year vs
// the current bill, clamped ≥ 0), and Hebrew reasons/caveats that explain WHY it
// ranked here — the same explainable output the app renders.
...
export function priorityFromId(id: string | undefined): MatchPriority {
  switch ((id ?? "").toLowerCase()) {
    ...
    case "no_commit":
      return "flexibility";
    case "price":
    case "data": // "big data volume" reads as value/price-leaning → balanced-price
      return "price";
    case "abroad":
    case "balanced":
    default:
...
// Base weights sum to 1.0; each priority re-tilts them. Ported verbatim from the
// Flutter engine's _weights so the app and the edge agent score IDENTICALLY.
function weightsFor(priority: MatchPriority): Weights {
  switch (priority) {
    ...
    case "speed":
      return { price: 0.20, saving: 0.16, rating: 0.14, speed: 0.34, coverage: 0.10, flex: 0.06 };
    case "coverage":
      return { price: 0.20, saving: 0.16, rating: 0.16, speed: 0.12, coverage: 0.30, flex: 0.06 };
    case "service":
      return { price: 0.20, saving: 0.18, rating: 0.36, speed: 0.10, coverage: 0.10, flex: 0.06 };
    case "flexibility":
      return { price: 0.24, saving: 0.20, rating: 0.14, speed: 0.08, coverage: 0.06, flex: 0.28 };
    case "balanced":
    default:
      return { price: 0.30, saving: 0.24, rating: 0.16, speed: 0.12, coverage: 0.10, flex: 0.08 };
  }
}
...
// proof. Keeps ratings out of today's ranking while leaving the path wired for a
// real-review pipeline. (Identical to the Dart _ratingSignal.)
...
function isGigFiber(plan: ScorablePlan): boolean {
  ...
  const hay = `${plan.plan ?? ""} ${(plan.feats ?? []).join(" ")} ${
    Object.values(plan.specs ?? {}).join(" ")
  }`;
  return (
    hay.includes("1000") || hay.includes("2000") || hay.includes("2500") ||
    hay.includes("5000") || hay.includes("גיגה") || hay.includes("1,000Mb") ||
...
// NEVER looks at plan.provider. Identical math to the Flutter engine, so the app
// and the edge agent produce the same 0..100 score + the same Hebrew reasons.
...
// ranks the same way on every surface (site, WhatsApp, app), which a live
// Math.random tie-break would break. Seed derives from the profile only — never
// from any provider — so it carries no brand bias.
...
export function bestMatch(plans: ScorablePlan[], profile: MatchProfile, opts: RankOptions = {}): PlanMatch | null {
