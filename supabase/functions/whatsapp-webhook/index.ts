...
// status inquiry routes to the agent (with the real lead context) instead of the
...
async function handleMessageInner(m: Row, profileName: string | undefined, aiKeys: AiKeys): Promise<void> {
  ...
  // APP-HANDOFF / OPEN-LEAD AWARENESS: the newest public.leads row for this
  // phone (any status — truth-only stage mapping happens in the agent). Looked up
  // ONLY for a free-text-ish turn (typed text or a transcribed voice note — the
  ...
  if (routeType === "audio" || routeType === "voice") {
  ...
  } else if (firstContact && !transcribed && !(openLead && isLeadStatusInquiry(routeText))) {
    ...
    // APP-HANDOFF: a first message that IS the app's prefilled status inquiry
    // (and a real lead exists for this phone) ALSO skips the welcome — the person
    // came to ask about THEIR request, so the agent (with the activeLead context)
    // answers with the real stage instead of a sales-y welcome menu. Guarded on
    // BOTH the lead existing AND the handoff pattern, so an ordinary first "היי"
    // still gets the welcome exactly as before.
    intent = "greeting";
    reply = WELCOME_REPLY;
    withMenu = true;
  } else {
    ...
    if (intent === "human") {
      ...
      const r = await runWhatsappAgent({
        ...
        // TRUTH-ONLY open-lead context (null → omitted → prompt unchanged): the
        // agent acknowledges the real stage on a status inquiry instead of
        // restarting a sales pitch; on any other question it's just available.
        activeLead: openLead ?? undefined,
        slotPatch: {
          ...(mergedCtx.category ? { category: mergedCtx.category } : {}),
          ...(mergedCtx.budget ? { budget: mergedCtx.budget } : {}),
